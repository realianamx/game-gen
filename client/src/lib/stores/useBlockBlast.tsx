...
import { Particle, createParticles, createExplosion, createSparkles, createBlastWave, createLightning, createShockwave, updateParticles } from '../../components/ParticleSystem';
  ...
  subscribeWithSelector((set, get) => ({
    ...
    placePiece: (piece: BlockShape, x: number, y: number) => {
      ...
      if (clearedLines.length === 1) {
        ...
        // Triple+ combo - EPIC lightning + blast wave + shockwave
        newParticles.push(...createBlastWave(centerX, centerY));
        ...
        newParticles.push(...createSparkles(centerX, centerY));
        newParticles.push(...createShockwave(centerX, centerY));
...
// Optimized particle animation loop with RAF
let particleAnimationFrame: number;
const animateParticles = () => {
  const state = useBlockBlast.getState();
  if (state.gameState.particles.length > 0) {
